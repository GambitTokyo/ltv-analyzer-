...
if uploaded is None and st.session_state.get('sample_df') is None:
    ...
    # ── How to use ───────────────────────────────────────────
    if get_lang() == 'ja':
        st.markdown("""<div class='section-title'>LTV Analyzer とは</div>""", unsafe_allow_html=True)
        st.markdown("""
LTV Analyzer は、顧客の生涯価値（LTV）と CAC（顧客獲得コスト）上限を、最小限の顧客データから自動算出する分析ツールです。平均単価・購入頻度・解約率などの単純な計算では見落とされる、まだ継続中の顧客が将来生む価値まで含めて算出します。
        """)
...
- 左サイドバーから 3 種類のサンプルデータ（**サブスク（日割りOFF）／サブスク（日割りON）／都度購入**）をダウンロードできます。顧客データを作成する際の参考にご活用ください。
        """)
...
**外れ値処理：** `revenue` が分布から極端に外れた顧客は推定結果を歪めます。サイドバーで上位パーセンタイルのクリッピング（例：上位 1%）を有効化することで高額側の外れ値を、下位パーセンタイルで `revenue` が 0 など不自然な低額側の顧客を除外できます。
        ...
        st.markdown("""
LTV Analyzer is an analytics tool that automatically computes customer lifetime value (LTV) and the CAC (customer acquisition cost) ceiling from a minimal customer data file. It captures the future value generated by still-active customers — value missed by simple calculations based on average revenue, purchase frequency, or churn rate.
        """)
...
- Three sample datasets (**subscription without proration / subscription with proration / spot purchase**) are downloadable from the left sidebar. Use them as a reference when preparing your own customer data.
        """)
