...
def _init_jp_font():
    ...
    if fp is None:
        for p in fm.findSystemFonts():
            if any(kw in p.lower() for kw in ['cjk','ipag','japanese','gothic']):
                fp = p; break
    if fp:
        fm.fontManager.addfont(fp)
        _JP_FONT_PROP = fm.FontProperties(fname=fp)
        plt.rcParams['font.family'] = _JP_FONT_PROP.get_name()
...
def _fp():
    return _JP_FONT_PROP or fm.FontProperties()
...
def _write_table(tbl, header, rows, footer=None):
    ...
    tmpl_row = copy.deepcopy(tbl.rows[1]._tr) if len(tbl.rows) > 1 else None
    while len(tbl.rows) < needed and tmpl_row is not None:
        tbl._tbl.append(copy.deepcopy(tmpl_row))
...
def _set_note_text(shape, note_text):
    ...
    first_para = paras[0] if paras else etree.SubElement(txBody, f'{{{A}}}p')
    for r in first_para.findall(f'{{{A}}}r'): first_para.remove(r)
    ...
    def _run(text, color, bold=True, sz=1000):
        r = etree.SubElement(first_para, f'{{{A}}}r')
    ...
    _run(prefix, '56B4D3', True)
    if body: _run(body, '3A6A7A', True)
...
def _set_s4_guide(shape, g):
    ...
    def _para(text, color='C8D0D8', bold=False, sz=1000):
    ...
    _para(f'・{g["lam_desc"]}', 'C8D0D8', False, sz)
    _para(f'・{g["k_desc"]}', 'C8D0D8', False, sz)
    _mpara([
        ...
        ('2年時点', '56B4D3', False), (f'で', 'C8D0D8', False),
        (f'{g["pct_2y"]:.1f}%', 'A8DADC', True), (f'（¥{g["ltv_2y"]:,.0f}）、 ', 'C8D0D8', False),
        ('3年時点', '56B4D3', False), (f'で', 'C8D0D8', False),
...
def _make_ltv_graph(t_range, rev_line, gp_line, cac_line, ltv_rev, lam_actual, x_max):
    ...
    for l in ax.get_yticklabels(): l.set_color('#888'); l.set_fontproperties(fp)
    ...
    ax.legend(
        ...
        ['LTV（売上）','LTV（粗利）','CAC上限'],
        loc='upper left', frameon=False, fontsize=7, labelcolor='white', ncol=3, bbox_to_anchor=(0.0,1.18), prop=fp)
...
def generate_pptx(
    ...
    if 'end_date' in df.columns and df['end_date'].notna().any(): _dcols.append(df['end_date'])
    _ds = __import__('pandas').concat(_dcols).dropna().min().strftime('%Y/%m/%d')
    _de = __import__('pandas').concat(_dcols).dropna().max().strftime('%Y/%m/%d')
    ...
    for sh in s4.shapes:
        ...
        elif sh.name == 'テキスト ボックス 3' and sh.has_text_frame and s4_guide_data:
            _set_s4_guide(sh, s4_guide_data)
    ...
    if not segment_cols_input.strip():
        ...
        for idx in [9,8,7,6,5]:
            if idx < len(sldIdLst):
                e = sldIdLst[idx]
                rId = e.get('{http://schemas.openxmlformats.org/officeDocument/2006/relationships}id')
                if rId:
        ...
        for sc in seg_cols:
            ...
            for ri, row in enumerate(pp_rows):
                ...
                try:
                    ...
                    for l in ax_km.get_xticklabels()+ax_km.get_yticklabels(): l.set_fontproperties(fp)
                    ...
                    for l in ax_wb.get_xticklabels()+ax_wb.get_yticklabels(): l.set_fontproperties(fp)
