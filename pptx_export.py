...
def _write_table_styled(tbl, hdr, rows, footer=None):
    ...
    for ri in range(len(data)):
        ...
        for ci in range(len(row.cells)):
            ...
            sf=etree.SubElement(tcPr,f'{{{A}}}solidFill'); c=etree.SubElement(sf,f'{{{A}}}srgbClr'); c.set('val',bg)
            for p in cell.text_frame.paragraphs:
                for r in p.runs:
                    rPr=r._r.find(f'{{{A}}}rPr')
                    if rPr is None: rPr=etree.SubElement(r._r,f'{{{A}}}rPr')
                    for sf2 in rPr.findall(f'{{{A}}}solidFill'): rPr.remove(sf2)
                    sf2=etree.SubElement(rPr,f'{{{A}}}solidFill'); c2=etree.SubElement(sf2,f'{{{A}}}srgbClr'); c2.set('val',fc)
...
def _set_note_text(sh, note_text):
    """NOTE色分け: endParaRPrをrunの後に移動、cap属性を除去"""
    if not sh.has_text_frame: return
    tf=sh.text_frame; txBody=tf._txBody
    p0=txBody.findall(f'{{{A}}}p')
    if not p0: return
    p0=p0[0]
    # テンプレートのrPrを保存（最初のrunから）
    tmpl=None
    for r in p0.findall(f'{{{A}}}r'):
        rp=r.find(f'{{{A}}}rPr')
        if rp is not None: tmpl=copy.deepcopy(rp); break
    # endParaRPrを保存して削除
    epr=p0.find(f'{{{A}}}endParaRPr')
    epr_copy=copy.deepcopy(epr) if epr is not None else None
    if epr is not None: p0.remove(epr)
    # 既存runを全削除
    for r in p0.findall(f'{{{A}}}r'): p0.remove(r)

    if ' — ' in note_text:
        prefix,body=note_text.split(' — ',1); body=' — '+body
    else: prefix,body=note_text,''

    def _run(text, color):
        r=etree.SubElement(p0,f'{{{A}}}r')
        if tmpl is not None:
            rPr=copy.deepcopy(tmpl)
            # cap属性を削除（全大文字変換を防ぐ）
            if 'cap' in rPr.attrib: del rPr.attrib['cap']
        else:
    ...
    _run(prefix,'56B4D3')
    if body: _run(body,'3A6A7A')
    # endParaRPrを最後に追加
    if epr_copy is not None: p0.append(epr_copy)
...
def _set_s4_guide(sh, g):
    ...
    # 3番目の箇条書き: LTV∞説明 + 改行 + 到達率（2行を1つの箇条書きに）
    _mpara([
        (f'LTV∞（¥{g["ltv_rev"]:,.0f}）は理論上の上限値で、実際にはこの金額に向かって時間をかけて積み上がります。','C8D0D8',False),
    ],sz,ind)
    # 到達率の行（同じ箇条書き内の2行目＝インデントなし箇条書きなし、marLだけ合わせる）
    p2=etree.SubElement(txBody,f'{{{A}}}p')
    pPr2=etree.SubElement(p2,f'{{{A}}}pPr'); pPr2.set('marL',str(ind)); pPr2.set('indent','0')
    buNone=etree.SubElement(pPr2,f'{{{A}}}buNone')
    ...
    _empty()
    _mpara([
        ('CAC設計の目安','56B4D3',True),('：回収期間に迷ったら、','C8D0D8',False),
...
def _make_ltv_graph(t_range, rev_line, gp_line, cac_line, ltv_rev, lam_actual, x_max):
    # 縦幅拡大: figsize heightを増やす
    fig,ax=plt.subplots(figsize=(898/150, 380/150), dpi=150)
    fig.patch.set_facecolor(BG); ax.set_facecolor(BG)
    # マーカー付き（間引き）
    mk=max(1,len(t_range)//30)
    ax.plot(t_range,rev_line,color='#56b4d3',lw=1.8,marker='o',markersize=2,markevery=mk)
    ax.plot(t_range,gp_line,color='#a8dadc',lw=1.8,ls='--',marker='o',markersize=2,markevery=mk)
    ax.plot(t_range,cac_line,color='#4a7a8a',lw=1.4,ls=':',marker='o',markersize=1.5,markevery=mk)
    ...
    ax.legend(
        [Line2D([0],[0],color='#56b4d3',lw=1.8,marker='o',ms=3),Line2D([0],[0],color='#a8dadc',lw=1.8,ls='--',marker='o',ms=3),Line2D([0],[0],color='#4a7a8a',lw=1.4,ls=':')],
        ['LTV (Revenue)','LTV (GP)','CAC Ceiling'],
        loc='upper left',frameon=False,fontsize=7,labelcolor='white',ncol=3,bbox_to_anchor=(0.0,1.15))
    ax.annotate(f'\u03bb={int(lam_actual)}d',xy=(lam_actual,ltv_rev*0.92),color='white',fontsize=7,annotation_clip=False)
    ax.annotate(f'LTV\u221e {ltv_rev:,.0f}',xy=(x_max+32,ltv_rev),color='#56b4d3',fontsize=7,va='center',annotation_clip=False,clip_on=False)
...
def generate_pptx(
    ...
    # S3用 k_insight / r2_comment（日本語に戻す）
    ...
    for h in horizons:
        ...
        else: lr=ltv_horizon_offset(k,lam,arpu_daily,h,ltv_offset_days); lg=ltv_horizon_offset(k,lam,gp_daily,h,ltv_offset_days)
