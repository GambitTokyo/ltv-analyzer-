...
# ── 日本語フォント：同梱ipag.ttfまたはシステムから探索 ──
_JP_FP = None
def _init():
    global _JP_FP
    # 1. リポジトリ同梱フォント
    _here = os.path.dirname(os.path.abspath(__file__))
    local_font = os.path.join(_here, 'ipag.ttf')
    # 2. システムフォント候補
    candidates = [
        local_font,
    ...
    for p in candidates:
        if os.path.exists(p):
            ...
            _JP_FP = fm.FontProperties(fname=p)
...
def _replace_image(sld, sh, buf):
    ...
    buf.seek(0)
    ip, rId = sld.part.get_or_add_image_part(buf)
...
def _write_table(tbl, hdr, rows, footer=None):
    ...
    while len(tbl.rows) < need and tr:
        tbl._tbl.append(copy.deepcopy(tr))
    while len(tbl.rows) > need and len(tbl.rows) > 1:
...
def _write_table_styled(tbl, hdr, rows, footer=None):
    ...
    for ri in range(len(data)):
        if ri + 1 >= len(tbl.rows): break
        row = tbl.rows[ri + 1]
        is_ft = footer and ri == len(data) - 1
        ...
        for ci in range(len(row.cells)):
            ...
            for p in cell.text_frame.paragraphs:
                for r in p.runs:
                    ...
                    if rPr is None:
                        rPr = etree.SubElement(r._r, f'{{{A}}}rPr')
                        r._r.insert(0, rPr)
...
def _set_note_text(sh, note_text):
    """NOTE色分け: テンプレートの既存runの書式を尊重しつつテキストだけ差し替え"""
    if not sh.has_text_frame: return
    tf = sh.text_frame
    ...
    if ' — ' in note_text:
        ...
        body = '\xa0— ' + body  # nbsp + em dash
    else:
        prefix, body = note_text, ''

    if len(runs) >= 2:
        # テンプレートに2run以上ある → 1つ目にNOTE、2つ目以降に本文
        runs[0].text = prefix
        # 1つ目のrun: 太字、色=3A6A7A（テンプレート通り、cap=allでNOTEは問題ない）
...
def _set_s4_guide(sh, g):
    ...
    def _para(text, color='C8D0D8', bold=False, sz=1000, indent=0):
        ...
        if indent > 0:
            ...
            bc = etree.SubElement(pPr, f'{{{A}}}buClr')
            c = etree.SubElement(bc, f'{{{A}}}srgbClr'); c.set('val', color)
    ...
    def _mpara(segs, sz=1000, indent=0):
        ...
        if indent > 0:
            ...
            pPr.set('marL', str(indent)); pPr.set('indent', str(-indent))
            bc = etree.SubElement(pPr, f'{{{A}}}buClr')
            c = etree.SubElement(bc, f'{{{A}}}srgbClr'); c.set('val', segs[0][1] if segs else 'C8D0D8')
            bs = etree.SubElement(pPr, f'{{{A}}}buSzPct'); bs.set('val', '100000')
            bu = etree.SubElement(pPr, f'{{{A}}}buChar'); bu.set('char', '•')
...
def _make_ltv_graph(t_range, rev_line, gp_line, cac_line, ltv_rev, lam_actual, x_max):
    ...
    # 特定ポイントのみマーカー
    marker_days = [180, 365, 1095, 1825, round(lam_actual)]
    marker_days = sorted(set(d for d in marker_days if d <= max(t_range)))
    def _find_y(line, day):
    ...
    # マーカー（特定ポイントのみ）
    for line, color in [(rev_line, '#56b4d3'), (gp_line, '#a8dadc'), (cac_line, '#4a7a8a')]:
        mx = marker_days
        my = [_find_y(line, d) for d in mx]
        ax.plot(mx, my, 'o', color=color, markersize=4, zorder=5)
