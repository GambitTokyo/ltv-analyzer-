...
_DICT = {
    ...
    'dormancy_90d':             {'ja': '90日', 'en': '90 days'},
    ...
    'main_step1_title':         {'ja': 'STEP 01 — 準備', 'en': 'STEP 01 — Prepare'},
    'main_step1_desc':          {'ja': 'CSV のアップロードとパラメータ設定', 'en': 'Upload your CSV and configure parameters'},
    'main_step2_title':         {'ja': 'STEP 02 — 分析', 'en': 'STEP 02 — Analyze'},
    'main_step2_desc':          {'ja': '統計手法を自動で実行', 'en': 'Statistical methods run automatically'},
    'main_step3_title':         {'ja': 'STEP 03 — 出力', 'en': 'STEP 03 — Export'},
    'main_step3_desc':          {'ja': 'Excel・PowerPoint・PDF にエクスポート', 'en': 'Export to Excel / PowerPoint / PDF'},
    ...
    'conclusion_spot_early_churn': {
        'ja': '期待される顧客生涯売上（LTV∞）—1顧客あたりの理論上の総売上—は{ltv}で、CAC上限は{cac}です。k={k:.3f}の初期離脱型で、初回購入後{period}以内に再購入しなかった顧客（単発購入）は{rate:.0f}%です。リピートした顧客の多くは初回購入からλ={lam:.0f}日（約{lam_y:.1f}年）以上購買を継続する傾向があります。投資回収は比較的長期になるため、暫定LTVテーブルで現実的な回収期間を確認してCACを設計してください。',
        'en': 'Expected lifetime revenue (LTV∞) — the theoretical total revenue per customer — is {ltv}, with a CAC cap of {cac}. With k={k:.3f}, this is an early-churn pattern: {rate:.0f}% of customers did not repurchase within {period} after their first purchase (one-time buyers). Repeat customers tend to continue purchasing for λ={lam:.0f} days (~{lam_y:.1f} years) or more. Payback takes relatively long — check the interim LTV table for realistic payback periods when designing your CAC.',
    },
    'conclusion_spot_late_churn': {
        'ja': '期待される顧客生涯売上（LTV∞）—1顧客あたりの理論上の総売上—は{ltv}で、CAC上限は{cac}です。k={k:.3f}の逓増離脱型で、初回購入後{period}以内に再購入しなかった顧客（単発購入）は{rate:.0f}%です。リピートした顧客の多くは初回購入からλ={lam:.0f}日（約{lam_y:.1f}年）以上購買を継続する傾向があります。比較的短期での投資回収が見込めます。',
        'en': 'Expected lifetime revenue (LTV∞) — the theoretical total revenue per customer — is {ltv}, with a CAC cap of {cac}. With k={k:.3f}, this is an increasing-churn pattern: {rate:.0f}% of customers did not repurchase within {period} after their first purchase (one-time buyers). Repeat customers tend to continue purchasing for λ={lam:.0f} days (~{lam_y:.1f} years) or more. Relatively short payback is expected.',
    },
    'conclusion_sub_early_churn': {
        'ja': '期待される顧客生涯売上（LTV∞）—1顧客あたりの理論上の総売上—は{ltv}で、CAC上限は{cac}です。k={k:.3f}の初期離脱型で、最初の契約期間のみで解約した顧客は{rate:.0f}%です。初期を乗り越えた顧客の多くはλ={lam:.0f}日（約{lam_y:.1f}年）以上継続する傾向があります。投資回収は比較的長期になるため、暫定LTVテーブルで現実的な回収期間を確認してCACを設計してください。',
        'en': 'Expected lifetime revenue (LTV∞) — the theoretical total revenue per customer — is {ltv}, with a CAC cap of {cac}. With k={k:.3f}, this is an early-churn pattern: {rate:.0f}% of customers churned within their first billing period. Those who survive the initial period tend to stay for λ={lam:.0f} days (~{lam_y:.1f} years) or more. Payback takes relatively long — check the interim LTV table for realistic payback periods when designing your CAC.',
    },
    'conclusion_sub_late_churn': {
        'ja': '期待される顧客生涯売上（LTV∞）—1顧客あたりの理論上の総売上—は{ltv}で、CAC上限は{cac}です。k={k:.3f}の逓増離脱型で、最初の契約期間のみで解約した顧客は{rate:.0f}%です。初期を乗り越えた顧客の多くはλ={lam:.0f}日（約{lam_y:.1f}年）以上継続する傾向があります。比較的短期での投資回収が見込めます。',
        'en': 'Expected lifetime revenue (LTV∞) — the theoretical total revenue per customer — is {ltv}, with a CAC cap of {cac}. With k={k:.3f}, this is an increasing-churn pattern: {rate:.0f}% of customers churned within their first billing period. Those who survive the initial period tend to stay for λ={lam:.0f} days (~{lam_y:.1f} years) or more. Relatively short payback is expected.',
    },
