...
_DICT = {
    ...
    'main_step1_title':         {'ja': 'STEP 01 — CSV をアップロードするだけ', 'en': 'STEP 01 — Drop your CSV and go'},
    'main_step1_desc':          {'ja': 'シンプルな顧客 CSV を用意 — ID、日付、売上のみ。個人情報は不要。ビジネスパラメータを設定して Analyze ボタン — 結果は数秒で表示。', 'en': 'Prepare a simple CSV — just customer IDs, dates, and revenue. No personal data required. Set your business parameters, hit Analyze — results appear in seconds.'},
    'main_step2_title':         {'ja': 'STEP 02 — 統計の専門家なしで、厳密な分析を', 'en': 'STEP 02 — Rigorous analysis — without the statistician'},
    'main_step2_desc':          {'ja': '外れ値検出、統計モデリング、LTV 投影 — 通常はデータサイエンスチームが必要な手法が自動で走ります。専門知識なしで厳密性を担保。', 'en': 'Outlier detection, statistical modeling, LTV projections — methods that normally require a data science team. They run automatically. You get the rigor without needing the expertise.'},
    'main_step3_title':         {'ja': 'STEP 03 — ビジネス相手にそのまま伝わるアウトプット', 'en': 'STEP 03 — Results your audience already understands'},
    'main_step3_desc':          {'ja': 'チャート、テーブル、セグメント比較 — すべてビジネス会話用にフォーマット済み。PowerPoint / PDF にワンクリック出力。次のミーティングにそのまま持ち込める。', 'en': 'Charts, tables, and segment comparisons — all formatted for business conversations. Export to PowerPoint or PDF in one click. Walk into your next meeting ready.'},
    'main_step_learn_more':     {'ja': '詳しく見る →', 'en': 'Learn more →'},
    'cta_pricing':              {'ja': 'レポート出力をご利用いただくには ltv-analyzer.com で購入 →', 'en': 'Get the full report — see pricing at ltv-analyzer.com →'},
